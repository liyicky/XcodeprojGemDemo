...
describe "AO_Xcodeproj" do
  ...
  describe "new_project" do
    ...
    it "should add a directory for the main target" do
      File.directory?(@project_path).should == true
    end
  end
  ...
  describe "new_project" do
    ...
    describe "add_gcov_flush" do
      before do
        File.new("#{@project_path}/#{@project.main_target.name}/#{@project.class_prefix}AppDelegate.m", "w+")
        @project.add_gcov_flush
      end

      it "should add gcov_flush() to the application's delegate" do
        app_delegate = File.open("#{@project_path}/#{@project.main_target.name}/#{@project.class_prefix}AppDelegate.m", "r").grep("\t__gcov_flush()")
      end
    end
  end
end
